...
let weather = {
 ...
 /* This is checking to see if the city name is already in the search history. If it is not, it will
 add it to the search history and create a button for it. */
  ...
  fetch(
    "https://api.openweathermap.org/data/2.5/forecast?q=" + city + "&units=imperial&cnt=5&appid=" + this.apiKey
  )
  .then((response) => response.json())
  .then((data) => {
...
/* This is a function that is being called when the button is clicked. It is logging the text content
 of the button and then calling the fetchWeather function. */
  search: function() {
...
/* This is an event listener that is listening for a click on the button. When the button is clicked,
it is calling the search function. */
